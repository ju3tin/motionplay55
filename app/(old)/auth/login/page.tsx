...
export default function LoginPage() {
  ...
  const handleWalletLogin = async () => {
    ...
    setWalletStatus('Connected: ' + wallet.publicKey.toString())

    try {
      // 1. Request nonce from your API
      ...
      if (verifyData.success) {
        setWalletStatus('Authentication successful!')
        router.push('/games') // redirect after successful wallet login
      } else {
        setWalletStatus('Authentication failed')
      }
    } catch (err) {
      setWalletStatus('Error during wallet login')
      console.error(err)
    }
  }
