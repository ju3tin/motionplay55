...
class DefaultDemoApi {
  ...
  process(imageData, timestamp) {
    ...
    return this.module_.process(
        {width: imageData.width, height: imageData.height, timestamp});
  }
  ...
  setRepeatedInputStreamStringValue(stream, value) {
    this.module_.setRepeatedInputStreamStringValue(stream, value);
  }

  setRepeatedInputStreamIntValue(stream, value) {
    this.module_.setRepeatedInputStreamIntValue(stream, value);
  }

  setRepeatedInputStreamFloatValue(stream, value) {
    this.module_.setRepeatedInputStreamFloatValue(stream, value);
  }

  resetRepeatedInputStreamValue(stream) {
    this.module_.resetRepeatedInputStreamValue(stream);
  }
...
scope.DemoApiPromise = new Promise((resolve) => {
  ...
  const demoModule = isWorker ? DemoModule : window.DemoModule;
  const module = isWorker ? Module : window.Module;
  demoModule(module).then((module) => {
    resolve({
      api: new DefaultDemoApi(module),
      serialization: isWorker ? new SerializationFactory(module) :
                                new window.SerializationFactory(module),
      VideoProcessor: isWorker ? null : window.VideoProcessor,
    });
  });
});
})(self);
